...
function SearchStations(){
    ...
    var searchprase = parent.querySelector("#stationsSearch").value;
        ...
        if (elements[i].textContent.toUpperCase().includes(searchprase.toUpperCase()) ||
            ...
            for (var j = 0; j < nested.length; j++) {
                if (nested[j].tagName === "LABEL" || nested[j].tagName === "INPUT") {
                    ...
                    elements[i].parentNode.hidden = true;
                }
            }
        }
    }
...
function Search(value) {
    var start = event.target;
    var parent = start.parentElement;
    var searchprase = parent.querySelector("#selectorsearch").value;
    var elements = parent.getElementsByTagName("LABEL")
...
function InitClick() {
    document.querySelectorAll("#selectorsbody").forEach(el=>{el.querySelectorAll("input[type=checkbox]").forEach(element => {
        element.addEventListener("change", Click, false);
    });})
}

function Seter(a,b){
    var fieldset = document.getElementById(b);
    var switcher = document.getElementById(a);
